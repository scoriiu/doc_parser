...
def compute_results(pdf_dir, keywords):
    ...
    for pdf_file in pdf_files:
        ...
        try:
            ...
            print(f'ERROR while processing {pdf_name}: {e}')
            results[f'{pdf_name} (ERROR)'] = {
                '#Patients': '',
                'Period Of Study': '',
                'AreaOfInterestMatched': False,
                **{k: '' for k in keywords},
            }
            continue
...
def export_to_excel(results, keywords, output):
    ...
    warn_format = {'font_color': 'orange', **basic_format}
    error_format = {'font_color': 'red', **basic_format}
    ...
    for k, v in results.items():
        doc_name = k
        doc_name += ' (*)' if not v['AreaOfInterestMatched'] and '(ERROR)' not in doc_name else ''
        row = [doc_name]
    ...
    for row_ix, row in enumerate(cells):
        for coll_ix, cell in enumerate(row):
            ...
            worksheet.write(row_ix + 1, coll_ix, cell, style)
